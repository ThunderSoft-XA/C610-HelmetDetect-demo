#ifndef SNPE_INFERENCE_HPP
#define SNPE_INFERENCE_HPP
...
public:
    InferSNPE(){};
    InferSNPE(std::string _model_path,DataFormat _format);
    ~InferSNPE();

    int object_num;

    RetState initRuntime(RunTime device);
    RetState deInit();
...
InferSNPE::InferSNPE(std::string _model_path,DataFormat _format)
{
 ...
 *   RunTime: Enumeration of supported target runtimes. 
 *   SNPE : The SNPE interface class definition .
 * */

RetState InferSNPE::initRuntime(RunTime device) {
...
template<typename T>
std::vector<T>  InferSNPE::loadByteDataMat(cv::Mat &_data_mat)
{
...
template<typename T>
bool  InferSNPE::loadByteDataMat(cv::Mat &_input_mat, std::vector<T>& loadVector)
{
    cv::Mat input;
    cv::Size src_size;
...
std::tuple<zdl::DlSystem::TensorMap, bool>  InferSNPE::loadMultipleInput (std::unique_ptr<zdl::SNPE::SNPE>& snpe , std::vector<cv::Mat>& mat_vec)
{
    ...
    for (size_t j = 0; j<inputTensorNames.size(); j++) {
        ...
        std::vector<unsigned char> inputVec = this->loadByteDataMat<unsigned char>(mat_vec[j]);
...
template <typename T>
std::vector<T> InferSNPE::doInference(std::vector<cv::Mat>& mat_vec) 
{
    assert(mat_vec.empty() != true);
...
void InferSNPE::setConfidence(float value) {
    mConfidenceThreshold = value > 1.0f ? 1.0f : (value < 0.0f ? 0.0f : value);
}

template <typename T>
std::vector<T> InferSNPE::doInference(cv::Mat object_mat)
{
