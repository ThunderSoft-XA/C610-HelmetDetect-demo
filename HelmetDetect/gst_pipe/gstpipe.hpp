...
public :
    ...
    virtual bool getFrameData(void* _pipe,cv::Mat& _dst) = 0;
...
void GstPipe::handleAppsink()
{
    ...
    // }
    g_signal_connect(this->appsink,"new-sample",G_CALLBACK(onBuffer),reinterpret_cast<void *>(this));
    g_signal_connect(this->appsink,"new-preroll",G_CALLBACK(onPreroll),reinterpret_cast<void *>(this));
    g_signal_connect(this->appsink,"eos",G_CALLBACK(onEOS),reinterpret_cast<void *>(this));
    printf ("all appsink callback was connected\n");
...
{
    printf ("enter gst on buffer callback\n");
    GstPipe *dec = NULL;
