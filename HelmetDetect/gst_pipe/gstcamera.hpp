...
void GstCamera::createPipeLine()
{
    ...
    if(isHwDec()) {
        cameraPath << getDecodeType() << "parse ! queue ! qtivdec ! qtivtransform ! video/x-raw,format=RGB ! "; //qtivtransform rotate=90CW
    } else {
...
bool GstCamera::getFrameData(void* _pipe,cv::Mat& _dst)
{
    printf("get gstreamer frame data\n");
    std::shared_ptr<GstSample> sample = NULL;
    GstCamera* pCam = reinterpret_cast<GstCamera *>(_pipe);

    pCam->getGstSample(sample);
    if(NULL == sample || NULL == sample.get()) {
        std::cout << "the camera pipe  sample is null or invaild" << std::endl;
        return false;
    }
    ...
    unsigned int*data_buffer = (unsigned int *)malloc(sizeof(unsigned int)*sample_map.size);
    cv::Mat sample_mat;
    if(data_buffer != nullptr) {
        ...
        (new cv::Mat(sample_height, sample_width, CV_8UC(sample_map.size/(sample_width * sample_height)), data_buffer, 0))->copyTo(_dst);	
    }
    std::cout << "orignal mat size: " << _dst.cols << " x " << _dst.rows << "in getFrameData"  << std::endl;
    free(data_buffer);
